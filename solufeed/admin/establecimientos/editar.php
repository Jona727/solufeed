...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!csrf_verify_post()) {
        ...
        try {
            ...
            flash_set('success', "Establecimiento '{$nombre}' actualizado correctamente.");
            header('Location: ' . $back_url);
            exit();
        } catch (Exception $e) {
            log_event('ERROR', 'ESTABLECIMIENTO_UPDATE_FAILED', ['error' => $e->getMessage()]);
            $error = 'Error al guardar el establecimiento. Reintentá o contactá soporte.';
        }
    }
}
?>
<?php
$page_title = 'Editar Establecimiento';
